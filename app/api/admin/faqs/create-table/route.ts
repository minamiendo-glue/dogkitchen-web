...
export async function POST(request: NextRequest) {
  try {
    ...
    // SQLを順番に実行
    const sqlCommands = [
      { name: 'テーブル作成', sql: createFAQTableSQL },
      { name: 'インデックス作成', sql: createIndexesSQL.join('\n') },
      { name: 'トリガー関数作成', sql: createTriggerFunctionSQL },
      { name: 'トリガー作成', sql: createTriggerSQL },
      { name: 'サンプルデータ挿入', sql: insertSampleDataSQL }
    ];
